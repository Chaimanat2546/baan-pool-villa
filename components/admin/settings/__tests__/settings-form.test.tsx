...
describe("SettingsForm", () => {
  ...
  it("links to the dedicated villa card image settings page", () => {
    const html = renderSettingsForm({
      ...DEFAULT_SITE_SETTINGS,
      villaCardStyle: "gallery",
    });

    expect(html).not.toContain('id="villa-card"');
    expect(html).not.toContain('data-villa-card-preview-option="classic"');
    expect(html).not.toContain('data-villa-card-preview-option="gallery"');
    expect(html).toContain('href="/admin/card-images"');
  });
