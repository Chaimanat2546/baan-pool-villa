...
export function mapContactSettingsResponse(value: unknown): ContactSettingsDraft {
  ...
  return { bankAccountName: bank.accountName, bankName: bank.bankName, bankAccountNumber: bank.accountNumber, phoneContacts: contact.phoneContacts.map((item) => ({ ...item })), messengerUrl: contact.messengerUrl, facebookPageName: contact.facebookPageName ?? "", showFacebookTimeline: contact.showFacebookTimeline, lineId: contact.lineId, lineUrl: contact.lineUrl };
}
