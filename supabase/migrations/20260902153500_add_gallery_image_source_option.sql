...
alter table public.site_web_styles
  ...
  check (
    ...
    or (
      style_type = 'gallery'
      and options - 'backgroundColor' - 'textColor' - 'categoryOrder' - 'showCover' - 'imageSource' = '{}'::jsonb
      ...
      and (not (options ? 'categoryOrder') or (case when jsonb_typeof(options -> 'categoryOrder') = 'array' then jsonb_array_length(options -> 'categoryOrder') = 11 and options -> 'categoryOrder' @> '["cover", "outside", "pool", "inside", "livingroom", "bedroom", "kitchen", "bathroom", "parking", "review", "uncategorized"]'::jsonb else false end))
    )
  );
